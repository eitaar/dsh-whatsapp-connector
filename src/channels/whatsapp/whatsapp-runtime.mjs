...
import {
  createEditableMessageStream,
  splitMessageText,
} from '../shared/editable-message-stream.mjs';
...
import { extractMentionTokens } from '../shared/text-harness-bridge.mjs';
import {
...
const WHATSAPP_STREAM_UPDATE_INTERVAL_MS = 5_000;
...
function messageMatchesTrigger(message, prefix) {
  if (!prefix) return true;
  const text = typeof message?.content === 'string' ? message.content : '';
  return text.replace(/^\s+/, '').toLowerCase().startsWith(prefix.toLowerCase());
}

function stripTriggerPrefix(message, prefix) {
  if (!prefix) return;
  const text = typeof message?.content === 'string' ? message.content : '';
  const rest = text.replace(/^\s+/, '');
  if (!rest.toLowerCase().startsWith(prefix.toLowerCase())) return;
  message.content = rest.slice(prefix.length).replace(/^\s+/, '');
}
...
export class WhatsappBotClient {
  ...
  async openStream(target, {
    updateIntervalMs = WHATSAPP_STREAM_UPDATE_INTERVAL_MS,
  } = {}) {
    let messageKey = null;
    const renderText = (text) => {
      const rendered = extractMentionTokens(text, target.senderId);
      return {
        text: rendered.text,
        ...(rendered.mentions.length ? { mentions: rendered.mentions } : {}),
      };
    };
    ...
    const create = async (text) => {
      const messageId = reserveMessageId();
      ...
      if (!result?.key?.id) throw new Error('WhatsApp did not return a stream message key');
      messageKey = result.key;
      rememberResult(result);
      return result.key.id;
    };
    const edit = async (_messageId, text) => {
      if (!messageKey) throw new Error('WhatsApp stream message key is unavailable');
      const messageId = reserveMessageId();
      const result = await this.#socket.sendMessage(
        target.jid,
        { ...renderText(text), edit: messageKey },
        { messageId },
      );
      rememberResult(result);
      return result;
    ...
    const stream = createEditableMessageStream({
      ...
      sendRemainder: (text) => {
        ...
        return this.sendText(
          { ...target, quoted: undefined },
          rendered.text,
          { mentions: rendered.mentions },
        );
      },
      messageIdForResult: (result) => result?.providerMessageIds?.[0],
    });
    await stream.start();
  ...
  async sendText(target, text, { mentions = [] } = {}) {
    ...
    const mentionList = Array.isArray(mentions)
      ? mentions.filter((jid) => typeof jid === 'string' && jid.endsWith('@s.whatsapp.net'))
      : [];
    for (const [index, chunk] of splitMessageText(text, 4_000).entries()) {
      ...
      const result = await this.#socket.sendMessage(
        target.jid,
        { text: chunk, ...(mentionList.length ? { mentions: mentionList } : {}) },
        options,
      );
      this.#outboundIds.remember(result?.key?.id);
...
export class WhatsappRuntime {
  ...
  setAccessPolicy(value) {
    ...
    this.#triggerPrefix = typeof policy.triggerPrefix === 'string' ? policy.triggerPrefix : '';
  ...
  #recordGroupMessage(message) {
    ...
    let arr = this.#groupContext.get(key);
    if (!Array.isArray(arr)) {
      arr = [];
      this.#groupContext.set(key, arr);
    }
    arr.push({
    ...
    this.#groupContext.set(key, arr.filter((entry) => entry.ts >= now - windowMs));
  }

  #buildGroupContext(conversationId, currentId) {
    const now = Date.now();
    const windowMs = 10 * 60 * 1000;
    const arr = this.#groupContext.get(conversationId);
    if (!Array.isArray(arr) || arr.length === 0) return '';
    const lines = arr
      .filter((entry) => entry.id !== currentId && entry.ts >= now - windowMs)
      .sort((a, b) => a.ts - b.ts)
      .map((entry) => {
        const label = typeof entry.sender === 'string' ? entry.sender.split('@')[0] : 'user';
        return `${label}: ${entry.text}`;
      });
    if (lines.length === 0) return '';
    return `[Group chat context — last 10 minutes]\n${lines.join('\n')}`;
  }
    ...
    try {
      const session = await this.#createSession({
        ...
        onMessage: async (raw, context) => {
          ...
          if (message.kind === 'group') this.#recordGroupMessage(message);
          ...
          // Trigger-prefix gate: when configured, only messages that begin with
          // the prefix are answered, and the prefix counts as "addressing" the bot
          // so it works for anyone (including the owner) in a group.
          if (this.#triggerPrefix && !messageMatchesTrigger(message, this.#triggerPrefix)) {
            this.#status.messagesRejected += 1;
            this.#status.lastRejectedAt = new Date().toISOString();
            return;
          }
          if (this.#triggerPrefix) {
            message.addressed = true;
            stripTriggerPrefix(message, this.#triggerPrefix);
          }
          if (message.kind === 'group') {
            message.groupContext = this.#buildGroupContext(message.conversationId, message.messageId);
          }
