...
function artifactFailureText(fileName, error) {
  ...
  switch (error?.code) {
    ...
    case 'artifact-permission-required':
      return t('结果文件「{name}」已生成，但当前机器人没有文件发送权限，请检查 WhatsApp 账号权限。', { name });
...
// Parses `@@me` / `@@sender` (resolved to the inbound sender JID) and
// `@@<phone>` (resolved to `<phone>@s.whatsapp.net`) tokens out of reply text,
// replacing them with the `@<number>` display form WhatsApp needs to render a
// native, tappable mention. Returns the cleaned text plus the JID list.
export function extractMentionTokens(text, senderId) {
  if (typeof text !== 'string' || !text) return { text: text ?? '', mentions: [] };
  const mentions = [];
  const cleaned = text.replace(MENTION_TOKEN, (match, token) => {
    let jid = null;
    if (token === 'me' || token === 'sender') {
      if (typeof senderId === 'string' && senderId.includes('@')) jid = senderId;
    } else {
      const number = token.replace(/^\+/, '');
      if (/^[1-9]\d{4,14}$/.test(number)) jid = `${number}@s.whatsapp.net`;
    }
    if (!jid || mentions.includes(jid)) return match;
    mentions.push(jid);
    return `@${jid.split('@')[0]}`;
  });
...
export class TextHarnessBridge {
  ...
  #acceptAcceptedMessage(normalized, messageId, senderId) {
    ...
    const approval = this.#approvals.claimReply({
      ...
      send: (text) => this.#deliverReply(normalized.replyTarget, text, senderId),
    });
    if (approval) {
      let task;
  ...
  #deliverReply(target, text, senderId) {
    const { text: clean, mentions } = extractMentionTokens(text, senderId);
    return this.#bot.sendText(target, clean, { mentions });
  }
  ...
  async #process(message, messageId, senderId, conversationKey, {
    ...
    const groupCtx = typeof message.groupContext === 'string' && message.groupContext
      ? message.groupContext : '';
    const llmText = groupCtx ? `${groupCtx}\n\n${text}` : text;
      ...
      if (!streamFinished) {
        try {
          const result = typeof this.#bot.sendDelivery === 'function'
            ...
            : await this.#deliverReply(target, visibleAnswer, message.senderId);
          textReceipt = createDeliveryReceipt({
            deliveryId: messageId,
