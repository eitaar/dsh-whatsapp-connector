...
<p align="center">
  <img src="assets/logo-dsh-im-connecting-readme-3x2.png" alt="连接 DeepSeek Harness" width="420" height="280">
...
机器人支持 Harness 会话、工作区、Agent Preset、模型和推理等级、批量输入、交互式问题、审批、文件回传，以及 `/help`、`/new`、`/status`、`/version`、`/models`、`/model`、`/reasoning`、`/preset`、`/stop`、`/steer`、`/batch`、`/send`、`/cancel`、`/compact`、`/workspace`、`/workspacelist`、`/sessionlist` 和 `/session` 等命令。机器人消息默认使用中文；设置 `language: en` 或 `DSH_IM_LANGUAGE=en` 后使用英文，包括流式状态 `Processing…`。
...
`npm run check` 会运行测试、构建 Host 与 Client 产物，并校验发布包。包身份保持为 `@xmanrui/dsh-im`，Cordis patch 继续保留 `xmanrui-dsh-im` 集成身份。
