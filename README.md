...
一个基于 [`@xmanrui/dsh-im`](https://github.com/xmanrui/dsh-im) 的 MIT 衍生项目，由 `eitaar` 维护，新增 WhatsApp 原生消息编辑与每 5 秒一次的流式进度更新。上游署名与第三方许可保留在 [`LICENSE`](LICENSE)、[`NOTICE.md`](NOTICE.md) 和 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)。
...
| WhatsApp | 使用手机 WhatsApp 扫码关联设备 | WhatsApp Web 长连接；默认仅响应账号自聊，也可切换到指定联系人或开放响应模式；显示已读和“正在输入”，并在生成过程中每 5 秒编辑同一条消息 |
