...
An MIT-licensed derivative of [`@xmanrui/dsh-im`](https://github.com/xmanrui/dsh-im) maintained by `eitaar`, adding native WhatsApp message editing for 5-second streaming progress updates. Upstream attribution and third-party notices are preserved in [`LICENSE`](LICENSE), [`NOTICE.md`](NOTICE.md), and [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md).

Install this GitHub snapshot with:
...
| WhatsApp | Scan a QR code with mobile WhatsApp to link a device | WhatsApp Web connection; self-chat only by default, with optional selected-contact and open-response modes; read receipt, typing indicator, and one message edited every 5 seconds while the answer is generated |
